...
if [ -z "${BASH_VERSION-}" ]; then
  ...
  if command -v bash >/dev/null 2>&1; then
    ...
    printf 'Example: \033[31;1mbash\033[0;34m -c "$(curl -fsSL https://raw.githubusercontent.com/MoulatiMehdi/oh-my-bash/master/tools/install.sh)"\n'
  fi
...
function _omb_install_print_version {
  ...
  printf '%s\n' 'Install script for Oh-My-Bash (https://github.com/MoulatiMehdi/oh-my-bash)'
...
export OSH='${OSH//\'/\'\\\'\'}'
  ...
  printf '%s\n' "${BLUE}${BOLD}To keep up on the latest news and updates, follow us on GitHub: https://github.com/MoulatiMehdi/oh-my-bash${NORMAL}"
...
function _omb_install_main {
  ...
  if [[ ! $OSH_REPOSITORY ]]; then
    OSH_REPOSITORY=https://github.com/MoulatiMehdi/oh-my-bash.git
  fi
